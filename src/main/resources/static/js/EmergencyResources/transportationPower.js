 ...
 * 运输力量统计
 */
var query_url = "TransportationPower/list?timestamp=" + Math.random();
var columns = {
	"transorg_name" : "机构名称",
	"belong_org_name" : "所属单位",
	"belong_org_level" : "单位级别",		
	"passenger_trans" : "客运能力/人",
	"goods_trans" : "货运能力/吨",
	"linkman_name" : "联系人姓名",
	"trans_equip_type" : "运输设备类型",
	"operation": "操作"
};
var conditions = {
		"pageSize" : 10
...
$(document).ready(function() {
	//导航默认选中
	$(".menusonFrist li").eq(7).addClass("active");
	...
	$.ajax({
		url : "TransportationPower/getStatistics?timestamp=" + Math.random(),
		type : 'get',
		data : {			
				},
		dataType : 'json',
		success : function(responseText, textStatus) {
			if (responseText.stateCode == "200") {
				...
				if(datas != undefined && datas != null){
					//运输机构客运能力统计
					if(datas[0] != null){
						setPic(datas[0], 'passengerCapacityBar', 'passengerCapacityPie', '运输机构客运能力统计', '运输机构客运能力比例图', '数量/人','人');
					}
					//运输机构货运能力统计
					if(datas[1] != null){
						setPic(datas[1], 'freightCapacityBar', 'freightCapacityPie', '运输机构货运能力统计', '运输机构货运能力比例图', '数量/吨','吨');
					}
				}else{
					//显示图片正在加载的页面
					loadPic('passengerCapacityBar', 'passengerCapacityPie');
					loadPic('freightCapacityBar', 'freightCapacityPie');					
				}
...
function createTable(tableDom, query_url, columns, conditions) {
	...
	for ( var i in columns) {
		...
		}else if(columns[i] == "客运能力/人"){
			aoColumns.push({
				"sTitle" : columns[i],
				"sWidth" : "102px"
			});
		}else if(columns[i] == "货运能力/吨"){
			aoColumns.push({
				"sTitle" : columns[i],
				"sWidth" : "102px"
			});
		}else if(columns[i] == "联系人姓名"){
			aoColumns.push({
				"sTitle" : columns[i],
				"sWidth" : "109px"
			});
		}else if(columns[i] == "运输设备类型"){
			aoColumns.push({
				"sTitle" : columns[i],
				"sWidth" : "115px"
			});
...
function rescueLookFunction(id) {
	...
	if (pageObj.list != undefined && pageObj.list != null) {
		for (var i in pageObj.list) {
			if (id == pageObj.list[i]["transorg_id"]) {
				flag = 1;
				break;
			}
		}
	...
	if(flag == 1){
		...
		//设置每个表单的值
		//机构名称
		$("#transportationName_check").val(data["transorg_name"]);	
		//单位级别
		$("#transportationLevel_check").val(data["belong_org_level"]);	
		//所属单位
		$("#transportationUnit_check").val(data["belong_org_name"]);
		//客运能力
		$("#passengerCapacity_check").val(data["passenger_trans"]);
		//货运能力
		$("#freightCapacity_check").val(data["goods_trans"]);
		//联系人姓名
		...
		//运输设备类型
		$("#transportationType_check").val(data["trans_equip_type"]);
		//经度
		$("#longitude_check").val(data["longitude"]);
		//纬度
		$("#latitude_check").val(data["latitude"]);	
		//地址
		$("#address_check").val(data["addr"]);		
		//基本情况
		$("#basisInfo_check").val(data["base_info"]);	
		//应急能力描述
		$("#emergencyDescribe_check").text(data["emer_ability"]);
		//备注
	...
	if(id=="tab1"){
		...
		if(loadFlag==0){
			...
			if(picData != undefined && picData != null){
				//运输机构客运能力统计
				if(picData[0] != null){
					setPic(picData[0], 'passengerCapacityBar', 'passengerCapacityPie', '运输机构客运能力统计', '运输机构客运能力比例图', '数量/人','人');
				}
				//运输机构货运能力统计
				if(picData[1] != null){
					setPic(picData[1], 'freightCapacityBar', 'freightCapacityPie', '运输机构货运能力统计', '运输机构货运能力比例图', '数量/吨','吨');
				}					
			}else{
				//显示图片正在加载的页面
				loadPic('passengerCapacityBar', 'passengerCapacityPie');
