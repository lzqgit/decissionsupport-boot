 ...
 * 消防力量统计
 */
var query_url = "FireFightingForce/list?timestamp=" + Math.random();
var columns = {
	"fireorg_name" : "机构名称",	
	"fireorg_no" : "机构代码",	
...
$(document).ready(function() {
	//导航默认选中
	$(".menusonFrist li").eq(4).addClass("active");
	...
	$.ajax({
		url : "FireFightingForce/getStatistics?timestamp=" + Math.random(),
		type : 'get',
		data : {			
				},
		dataType : 'json',
...
function rescueLookFunction(id) {
	...
	if (pageObj.list != undefined && pageObj.list != null) {
		for (var i in pageObj.list) {
			if (id == pageObj.list[i]["fireorg_id"]) {
				flag = 1;
				break;
			}
		}
	...
	if(flag == 1){
		...
		//设置每个表单的值
		//机构名称
		$("#institutionName_check").val(data["fireorg_name"]);	
		//机构代码
		$("#institutionNum_check").val(data["fireorg_no"]);
		//负责人姓名
		...
		//急救车辆类型
		$("#ambulanceType_check").val(data["rescue_car_type"]);
		//经度
		$("#longitude_check").val(data["longitude"]);	
		//纬度
		$("#latitude_check").val(data["latitude"]);	
		//机构详细地址
		$("#institutionAddress_check").text(data["fireorg_addr"]);
		//机构详细介绍
		$("#institutionInfo_check").text(data["fireorg_desc"]);		
	}
}
/*通过点击按钮加载图表数据*/
function liClick(id) {
